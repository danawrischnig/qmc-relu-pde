...
The mesh resolution is controlled by ``--level L``.

with $2^L$ subdivisions per coordinate direction.

### Boundary condition
In both problems, the boundary data $b \in \mathbb R$ is chosen as a function of the PDE parameters such that less than $1%$ of the QMC values are positive. 
...
$$
I(g) = \int_{[0,1)^s} \operatorname{ReLU}(g(y)) \mathrm dy
$$
